...
#endif
{
	...
	ClipSurface(ScreenPos, input.positionCS.z, vFace);
	#endif
	
	//return float4(depth.linear01, depth.linear01, depth.linear01, 1);

	#if _REFRACTION
	float4 refractedScreenPos = ScreenPos.xyzw + (float4(worldTangentNormal.xz, 0, 0) * (_RefractionStrength * 0.1));
	#endif
	...
	DepthData depth = SampleDepth(ScreenPos);
	...
	DepthData depthRefracted = SampleDepth(refractedScreenPos);
...
#if !_DISABLE_DEPTH_TEX
	#ifdef CAMERA_DEPTH_ABSORPTION
	AbsorptionDepth = 1-saturate(depth.raw / _Depth * 0.5);
	AbsorptionDepth = max(AbsorptionDepth, saturate(1-(exp(-aborptionDist) / _Depth)));
	#else
	AbsorptionDepth = saturate(lerp(aborptionDist / _Depth, 1-(exp(-aborptionDist) / _Depth), _DepthExp));
	#endif
	#if UNDERWATER_ENABLED
	AbsorptionDepth = lerp(1, AbsorptionDepth, vFace);
	#endif
#endif
...
#if _CAUSTICS
	float3 caustics = SampleCaustics(opaqueWorldPos.xz, TIME * _CausticsSpeed, _CausticsTiling) * _CausticsBrightness;
	#if _ADVANCED_SHADING
	...
	translucencyData = PopulateTranslucencyData(_ShallowColor.rgb, mainLight.direction, mainLight.color, viewDirNorm, waveNormal, worldTangentNormal, transmissionMask, _TranslucencyParams);
	#if UNDERWATER_ENABLED
	...
	surfaceData.smoothness = sunSpec.a;
	...
	#if defined(DEBUG_DISPLAY) && VERSION_GREATER_EQUAL(12,0)
